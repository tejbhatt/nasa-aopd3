...
class DailyImage extends Component {
	...
	handleNextDayClick = () => {
		...
		if (today.date() === moment().date()) {
		    ...
		    url = `https://localhost:8001/getApod?api_key=${process.env.REACT_APP_NASA_APOD_KEY}&date=${storedDate}`;
		} else {
			this.props.setCurrentDate(today.add(1, 'days'));

			let tomorrow = today.format('YYYY-MM-DD');

		    url = `https://localhost:8001/getApod?api_key=${process.env.REACT_APP_NASA_APOD_KEY}&date=${tomorrow}`;
		}
